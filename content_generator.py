...
def generate_content_plan(df_top: pd.DataFrame, target_reader: str, user_strength: str, api_key: str) -> str:
    ...
    system_prompt = "あなたはプロのコンテンツマーケターであり、note専門の凄腕編集者です。データに基づき、読者の心を動かし競合と差別化できる連載・複数記事のアイデアを作成してください。"

    # 【改善点】中途半端な目次生成を廃止し、「すぐに使える専用のAIプロンプト」を出力させるように大幅修正
    user_prompt = f"""
    ...
    【重要】各テーマについて、必ず以下のフォーマットで「売れる理由」「タイトル案」「専用プロンプト」のセットを出力してください。途中で絶対に省略しないでください。
    ...
    **🤖 この記事をAIに書かせるための専用プロンプト**
    （※ユーザーがChatGPT等にそのままコピペして使えるように、以下のコードブロック形式で出力してください）
    ```text
    ...
    ・記事のゴール：読者の悩みを解決し、次の行動（有料記事の購入やフォロー等）へ促すこと
    ...
    ---
    ### 【テーマ2】: [具体的なテーマ名や切り口]
    （※テーマ1と全く同じフォーマットで、理由・タイトル・専用プロンプトを出力）
